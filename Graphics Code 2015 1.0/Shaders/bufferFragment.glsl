...
const vec3 grass=vec3(0.6,0.6,0.6);
void main ( void ) {

  float floor=clamp((IN.worldPos.y-250.f)/130.f,0.f,1.f);

	vec4 diffuse = texture(diffuseTex,IN.texCoord);
  vec3 floor_grass=mix(grass,diffuse.rgb,floor);
...
gl_FragColor [0] =vec4(floor_grass*shadow,diffuse.a);
